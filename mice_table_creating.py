...
class MainApp:
    ...
    def open_parameter_window(self):
        ...
        # Setup and start serial reading thread
        self.stop_event.clear()
        self.serial_thread = threading.Thread(target=self.read_from_serial, daemon=True)
        self.serial_thread.start()

        # Start periodic GUI update from queue
        self.master.after(100, self.process_serial_queue)

        # Handle window close properly
        self.parameter_window.protocol("WM_DELETE_WINDOW", self.on_close)

        self.parameter_window.wait_window()  # Modal-like behavior
    ...
    def process_serial_queue(self):
        ...
        while not self.serial_queue.empty():
            latest_line = self.serial_queue.get()

        if latest_line is not None:
            self.data_display.config(state=tk.NORMAL)
            self.data_display.delete("1.0", tk.END)  # מחיקה של כל הטקסט הקודם
            self.data_display.insert(tk.END, latest_line + "\n")
            self.data_display.yview(tk.END)
            self.data_display.config(state=tk.DISABLED)

        if self.parameter_window.winfo_exists():
            self.master.after(100, self.process_serial_queue)
    ...
    def create_mice_table(self):
        ...
        if self.mice_list:
            ...
            for i, item in enumerate(self.mice_list):
                ...
                option_var = tk.StringVar(value=str(self.main_GUI.levels_list[0]))
                OptionMenu = ttk.OptionMenu(self.miceTableFrame, option_var, self.main_GUI.levels_list[0],
                                           *self.main_GUI.levels_list)
                OptionMenu.grid(row=i + 1, column=1, sticky="nsew", padx=5, pady=2)

                self.option_vars.append(option_var)

            self.miceTableFrame.grid_columnconfigure(0, weight=1)
            self.miceTableFrame.grid_columnconfigure(1, weight=0)
            for row in range(len(self.mice_list) + 1):
                self.miceTableFrame.grid_rowconfigure(row, weight=0)
            self.set_mice_as_dict()

    def set_mice_as_dict(self):
        data = {}
        for i in range(len(self.mice_list)):
            mouse_label = self.miceTableFrame.grid_slaves(row=i + 1, column=0)[0]
            selected_level = self.option_vars[i].get()
